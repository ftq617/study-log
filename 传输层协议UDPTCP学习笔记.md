...
6. TCP连接，三次握手
    ...
    ```
    * 注意：
        * x,y 为指定的开始发送序号，可以自己设置
        * 第三次的握手是可以携带数据的，携带数据的话 seq= x + 携带数据大小的字节数
        * 第三次的握手如果不携带数据话，报文头seq=x+1，但是这个x+1 不占用一个字节，下一个数据报还是 从x 开始
7. TCP释放，四次挥手
    ```
            客户端                                    服务端
                  |   FIN=1   seq=x                         |
                  |---------------------------------------> |   
                  |   ACK=1  seq=y  ack=x+1                 |
                  | <---------------------------------------|   收到释放连接的数据报，需要立马返回确认收到
                  |   FIN=1  ACK=1  seq=y+z  ack=x+1        |  
                  | <---------------------------------------|   这里负责将还没有传输完的数据传输给客户端
                  |   ACK=1  seq=x+1 ack=y+1+z              |
                  | --------------------------------------->| CLOSE     这里返回数据收到的确认报文
                  |
                  |等待两个最长报文段寿命 MSL(Maximun Segment Lifetime)     这里等待的原因是为了避免 服务端没有收到确认报文，然后服务端进行重传，客户端再确认（并重置这个等待时间），一来一回正好是两个最大报文存活时间
                  | CLOSE
    ```
    * x 等于客户端之前传输的数据字节加1
    * y 等于服务端之前输出的数据字节加1
    * z 等于服务端还要发送给客户端的字节数
    * 如果最后客户端不等两个最长报文段寿命，那么服务端没收到确认报文就不能正常退出，会一直在重传报文。
        
    * 保活计时器:如果客户端发送了死机，没有释放连接，那么服务端就会一直在等待。为了不让服务端白白的等待，TCP设有一个保活计时器，每收到一次数据就重置一下保活计时器，时间通常是两小时，如果两小时没收到数据就会发送一个探测报文段，之后每隔75秒发送一个次，如果连续10次都没回应，则认为对方出了故障，接着关闭这个连接。
### TCP 和 UDP 的 区别
